...
#pragma region CollisionFunctionality
	//GetDistance function to calculate distance between 2 points
	...
	//Functions to check if a point is within a circle
	...
	{
		return IsPointInCircle(point, Circlef(center, radius));
	}
	bool IsPointInCircle(const Point2f& point, float circleX, float circleY, float radius)
	{
		return IsPointInCircle(point, Circlef(circleX, circleY, radius));
	}
	bool IsPointInCircle(float pointX, float pointY, const Circlef& circle)
	{
		return IsPointInCircle(Point2f(pointX, pointY), circle);
	}
	bool IsPointInCircle(float pointX, float pointY, float circleX, float circleY, float radius)
	{
		return IsPointInCircle(Point2f(pointX, pointY), Circlef(circleX, circleY, radius));
	}

	//Functions to check if a point is within a rectangle
	...
	{
		return IsPointInRect(point, Rectf(left, top, width, height));
	}
	bool IsPointInRect(float pointX, float pointY, const Rectf& rect)
	{
		return IsPointInRect(Point2f(pointX, pointY), rect);
	}
	bool IsPointInRect(float pointX, float pointY, float left, float top, float width, float height)
	{
		return IsPointInRect(Point2f(pointX, pointY), Rectf(left, top, width, height));
	}

	//Check wether 2 shapes are overlapping each other
